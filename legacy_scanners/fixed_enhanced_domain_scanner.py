#!/usr/bin/env python3
"""
FIXED Enhanced Domain Discovery System
SSL-fixed version with working Anthropic integration
"""
...
class FixedLLMPublisherDiscovery:
    """LLM discovery with SSL fix"""
    ...
    def _setup_client(self):
        """Setup Anthropic client with SSL bypass"""
        ...
        try:
            ...
            logger.info("✅ Fixed Anthropic client initialized (SSL bypass)")
            
        except Exception as e:
            logger.error(f"Failed to setup fixed Anthropic client: {e}")
            self.client = None
...
Category: {category_prompts.get(category, category)}"""
    ...
    async def expand_from_existing(self, known_domains: List[str], count: int = 30) -> List[str]:
        ...
        if not self.client or not known_domains:
            return []
...
class FixedEnhancedDomainScanner:
    """Enhanced domain scanner with fixed LLM integration"""
    
    def __init__(self, db_path: str = "fixed_domain_discovery.db"):
        self.db_path = db_path
        self.session = None
        self.validator = RobustDomainValidator()
        self.llm_discovery = FixedLLMPublisherDiscovery()
    ...
    def setup_database(self):
        """Setup enhanced database with detailed scoring tracking"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        # Main tracking table with detailed scoring breakdown
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS domains_analyzed (
                ...
                current_score REAL,
                strategy_used TEXT,
                discovery_source TEXT DEFAULT 'llm_fixed',
                ...
                b2b_relevance REAL DEFAULT 0,
                -- Ads.txt analysis
                has_ads_txt BOOLEAN DEFAULT FALSE,
                total_ads_entries INTEGER DEFAULT 0,
                direct_deals INTEGER DEFAULT 0,
                reseller_deals INTEGER DEFAULT 0,
                premium_platforms INTEGER DEFAULT 0,
                premium_platform_list TEXT DEFAULT '',
                -- Rejection details
        ...
        conn.commit()
        conn.close()
        logger.info("✅ Enhanced database with detailed scoring initialized")
    ...
    async def get_session(self):
        ...
        if self.session is None:
            ...
            self.session = aiohttp.ClientSession(
                timeout=timeout,
                headers={'User-Agent': 'Mozilla/5.0 (compatible; FixedDomainScanner/1.0)'}
            )
        return self.session
    ...
    async def intelligent_domain_discovery(self, count: int = 100) -> List[str]:
        ...
        logger.info(f"🧠 Fixed LLM discovering {count} high-quality publishers...")
    ...
    def save_analysis(self, result: ScanResult):
        ...
        # Extract detailed scoring information from result
        details = result.details
        content_details = details.get('content_details', {})
        ads_analysis = details.get('ads_analysis', {})
        
        cursor.execute('''
            INSERT OR REPLACE INTO domains_analyzed 
            (domain, first_analyzed_at, last_analyzed_at, analysis_count, 
             current_status, current_score, strategy_used, discovery_source, validated, validation_time,
             content_score, has_content, ad_slots_detected, quality_indicators, b2b_relevance,
             has_ads_txt, total_ads_entries, direct_deals, reseller_deals, premium_platforms, premium_platform_list,
             rejection_reason, detailed_analysis)
            VALUES (?, 
        ...
        ''', (
            ...
            details.get('discovery_source', 'llm_fixed'),
            details.get('validated', False),
            details.get('validation_time', 0),
            # Content scoring details
            details.get('content_score', 0),
            content_details.get('has_content', False),
            content_details.get('ad_slots_detected', 0),
            content_details.get('quality_indicators', 0),
            content_details.get('b2b_relevance', 0),
            # Ads.txt details
            details.get('ads_txt_found', False),
            ads_analysis.get('total_entries', 0),
            ads_analysis.get('direct_deals', 0),
            ads_analysis.get('reseller_deals', 0),
            details.get('premium_dsps', 0),
            ','.join(ads_analysis.get('premium_platforms', [])),
            # Rejection/analysis details
            result.reason if result.status == 'rejected' else '',
            json.dumps({
                'content_details': content_details,
                'ads_analysis': ads_analysis,
                'scoring_breakdown': {
                    'content_score': details.get('content_score', 0),
                    'ads_txt_bonus': 25 if details.get('ads_txt_found', False) else 0,
                    'premium_bonus': details.get('premium_dsps', 0) * 3,
    ...
    async def enhanced_scan_domain(self, domain: str, strategy: str = 'flexible', 
                                 discovery_source: str = 'llm_fixed') -> Optional[ScanResult]:
        ...
        try:
            ...
            if content_score < 20:  # Must have some content
                ...
                status = 'approved' if final_score >= 40 else 'rejected'
...
async def run_fixed_discovery_pipeline(target_count: int = 20) -> Dict:
    """Fixed discovery pipeline with working LLM"""
    scanner = FixedEnhancedDomainScanner()
    
    try:
        print("🚀 FIXED ENHANCED DOMAIN DISCOVERY PIPELINE")
        print("=" * 50)
        
        # Step 1: LLM Discovery (with SSL fix)
...
async def main():
    ...
    result = await run_fixed_discovery_pipeline(target_count=15)
    print(f"\n🎉 Fixed pipeline result: {result}")

if __name__ == "__main__":
    print("🚀 Fixed Enhanced Domain Discovery System")
    print("SSL-bypass enabled for Anthropic API\n")
